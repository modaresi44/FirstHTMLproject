...
function onPlayerStateChange(event, ytPlayer) { 
  ...
  videoID = matches[1];
  thisVideoTitle = (ytPlayer.getVideoData().title) ? ytPlayer.getVideoData().title : videoID;
    ...
    if (videoArray[j]==videoID) {
      ...
      //should the video tire out and cease
          if (event.data == YT.PlayerState.ENDED) {
            parent.$('body').trigger("youtube", {
              state: "ended",
              title: thisVideoTitle,
              video_id: videoID
            });
